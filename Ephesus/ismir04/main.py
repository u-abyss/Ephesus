...
# if track_ids_in_order[128] in target_category_track_ids:
#     print("yes")

# goal_indexの対象となるもの
def get_goal_indexes(track_ids):
    goal_indexes = []
    for track_id in track_ids:
        index = track_ids_in_order.index(track_id)
        goal_indexes.append(index)
    return goal_indexes

goal_indexes = get_goal_indexes(target_category_track_ids)
...
start_indexes = get_start_indexes(source_category_track_ids)
print(start_indexes)
...
for goal_index in goal_indexes:
    ...
    arr.append([index_weight_list, passed_index])
...
def run_algorithm(index_weight_list):
    # ノードの数
...
# run_algorithm(index_weight_list)
